...
WORKDIR /lambda-watcher
RUN sudo chown -R nuser:nuser /lambda-watcher/

# Create Gemset
RUN /bin/bash -l -c "rvm gemset create lambda-watcher"
RUN /bin/bash -l -c "rvm use 2.2.3@lambda-watcher --default"
...
WORKDIR /lambda-watcher/worker
RUN /bin/bash -l -c "npm install"
WORKDIR /lambda-watcher

VOLUME ["/lambda-watcher"]
